...
func (a *ApplicationTestSuit) TestRegister_Success() {
	...
	a.user.EXPECT().Register(ctx, "John", "Doe", "john.doe@example.com").Return(expectedUserID, nil)
	a.hashManager.EXPECT().HashPassword(password).Return(hashedPassword, nil)
	a.DB.EXPECT().Create(ctx, gomock.Any()).Return(nil, nil)
	a.authManger.EXPECT().GenerateVerificationCode(ctx, expectedUserID).Return(verificationCode, nil)
...
func (a *ApplicationTestSuit) TestRegister_GenerateVerificationCodeError() {
	...
	a.hashManager.EXPECT().HashPassword(password).Return(hashedPassword, nil)
	a.DB.EXPECT().Create(ctx, gomock.Any()).Return(nil, nil)
	a.authManger.EXPECT().GenerateVerificationCode(ctx, expectedUserID).Return("", ErrUnknownError)
...
func (a *ApplicationTestSuit) TestRegister_SendVerificationCodeError() {
	...
	a.DB.EXPECT().Create(ctx, gomock.Any()).Return(nil, nil)
	a.authManger.EXPECT().GenerateVerificationCode(ctx, expectedUserID).Return(verificationCode, nil)
	a.email.EXPECT().SendVerificationCode("john.doe@example.com", verificationCode).Return(ErrUnknownError)
...
func (a *ApplicationTestSuit) TestVerifyEmail_Success() {
	...
	a.authManger.EXPECT().CompareVerificationCode(ctx, userID, verificationCode).Return(true, nil)
	a.DB.EXPECT().VerifyEmail(ctx, userID).Return(nil, nil)

	err := a.api.VerifyEmail(ctx, userID, verificationCode)
	a.NoError(err)
}
...
func (a *ApplicationTestSuit) TestVerifyEmail_CompareVerificationCodeError() {
	...
	a.authManger.EXPECT().CompareVerificationCode(ctx, userID, verificationCode).Return(false, ErrUnknownError)
